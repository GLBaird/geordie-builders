...
function copyFiles(output, settings) {
    ...
    for (var i in files) {
        if (files.hasOwnProperty(i)) {
            var src = path.join( "./templates", files[i] );
    ...
    var bowerPath = bootstrap ? "./templates/bower-bootstrap.json" : "./templates/bower-nobootstrap.json";
    ...
    fs.writeFileSync(mainCSSFile, fs.readFileSync("./templates/src/css/01_main.css"));
    success = success && fs.existsSync(mainCSSFile);

    // load templates for view related copies
    var unitTemplate = fs.readFileSync("./templates/tests/controller-spec.js").toString('utf8');
    var controllerTemplate = fs.readFileSync("./templates/src/js/controllers/controller-template.js").toString('utf8');
    var viewTemplate = fs.readFileSync("./templates/src/js/views/view-template.html").toString('utf8');
    var controllerRegistry = fs.readFileSync("./templates/src/js/controllers/index.js").toString('utf8');
    var cRegister = "";
    var routers = fs.readFileSync("./templates/src/js/routes/index.js").toString('utf8');
    var routeRegistry = "";
    var otherwise;
